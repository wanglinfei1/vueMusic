<template>
  ...
  >
    <ul class="suggest-list">
      <li class="suggest-item" v-for="item in result" @click="selectItem(item)" :key="item.key">
        <div class="icon">
...
export default {
  ...
  methods: {
    ...
    searchMore() {
      ...
      this.page++
      getSearch(this.query, this.page, this.perpage, this.showSing).then(
        res => {
          if (ERR_OK === res.code) {
    ...
    _normaliszeSongs(list, _result, data) {
      ...
      this._CgiGetVkey(ids, types, list, 'suggest')
        .then(midurlJson => {
